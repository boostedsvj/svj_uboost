...
def bdt_ddt_inputs(input_files: list[str], all_features, apply_RT_cut=True):
    def _get_cols(file: str):
        return apply_rt_signalregion(Columns.load(file)) if apply_RT_cut else Columns.load(file)
...
def main():
    ...
    # Evaluation of the target cuts values, here we do apply the RT cuts
    ...
    if '2D_DDT_map' in plots:
        for key in var_dict.keys():
            ...
            var_map_smooth, MT_PT_edges, PT_edges, RT_edges = var_dict[key]
            var_map_smooth = np.array(var_map_smooth)[:,:,-1] # Only plotting the signal region for now
    ...
    if 'bkg_scores_mt' in plots :
        ...
        fig, ax = plt.subplots(figsize=(10, 8))
        hep.cms.label(rlabel="(13 TeV)")
        bin_edges = np.linspace(180, 650, 48)  # 50 bins
    ...
    if 'fom_significance' in plots :
        ...
        for mz, mz_files in files_by_mass.items():
            ...
            sig_X, sig_pT, sig_mT, sig_rT, sig_weight = bdt_ddt_inputs(mz_files, ana_variant[ana_type]['features'])
    ...
    if ana_type == 'BDT-based' :
        ...
        if 'sig_mt_single_BDT' in plots :
            ...
            for sig_file in expand_wildcards([sig_files]):
                ...
                sig_score_ddt = calculate_varDDT(sig_mT, sig_pT, sig_rT, sig_score, sig_weight, sig_bdt_cut, ddt_map_file)

                # Make mT distributions
                if mz != 300: alpha = 0.3
                else: alpha = 1.0
                score_mask = sig_score_ddt > 0.0 # DDT score > 0.0 is equivalent to BDT score about BDT cut value
                ax.hist(sig_mT[score_mask], weights=sig_weight[score_mask], bins=47, range=(180,650), histtype='step', label=f"m(Z')={mz}", alpha=alpha)
        ...
        if 'one_sig_mt_many_bdt' in plots :
            ...
            for cuts, scores in zip(ana_variant[ana_type]['cut_values'], sig_score_ddt):
                score_mask = scores > 0.0 # DDT score > 0.0 is equivalent to BDT score about BDT cut value
                ax.hist(sig_mT[score_mask], weights=sig_weight[score_mask], bins=47, range=(180,650), histtype='step', label=f'DDT(BDT cut = {cuts})')
            ...
            for cuts, scores in zip(ana_variant[ana_type]['cut_values'], sig_score_ddt):
                score_mask = scores > 0.0 # DDT score > 0.0 is equivalent to BDT score about BDT cut value
                ax.hist(sig_mT[score_mask], weights=sig_weight[score_mask], bins=47, range=(180,650), histtype='step', label=f'DDT(BDT cut = {cuts})', density=True)
